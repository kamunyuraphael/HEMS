...
# Placeholder user ID — replace with real MongoDB ObjectId from your
# authenticated user once the frontend auth flow is complete
DEFAULT_USER_ID = Config.__dict__.get("DEFAULT_USER_ID", "000000000000000000000001")
...
def _train_models() -> bool:
    ...
    try:
        ...
        # Disaggregation model — CO algorithm trains directly on elec object
        _disaggregator.train(train_start=TRAIN_START, train_end=train_end)

        # Get disaggregated power for forecaster and anomaly training
...
def run_pipeline() -> None:
    ...
    # Use a window ending yesterday so we always have complete-day data
    window_end = datetime.now().strftime("%Y-%m-%d")
    window_start = (
        datetime.now() - timedelta(days=INFERENCE_WINDOW_DAYS)
    ).strftime("%Y-%m-%d")

    # For UK-DALE (historical dataset), use fixed recent dates
    # In production with live sensor data, use real datetime.now() windows
    ...
    try:
        ...
        # Disaggregate the ingestion window
        power_df = _disaggregator.disaggregate(
