...
class ConsumptionForecaster:
    ...
    def _build_features(self, power_df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["lag_1"] = agg.shift(1).fillna(method="bfill")
        df["lag_10"] = agg.shift(10).fillna(method="bfill")
        df["lag_600"] = agg.shift(steps_1h).fillna(method="bfill")
    ...
    def forecast(
        ...
        Strategy: build features for each hour of the target date using
        the recent_power_df as context for rolling and lag features, then
        predict with each appliance's trained model.
        ...
        # Build a synthetic index for the 24 hours of the target date
        ...
        # Build features from recent data + append the target hours
        extended_df = pd.concat([
            recent_power_df,
            pd.DataFrame(index=target_index, columns=recent_power_df.columns, dtype=float).fillna(0),
        ])

        features_6s = self._build_features(extended_df)
        features_hourly = self._align_to_hourly(features_6s)

        # Extract only the target date's hourly rows
        target_features = features_hourly[
            features_hourly.index.normalize() == pd.Timestamp(target_date).normalize()
        ]

        if target_features.empty:
            # Fallback: use the last 24 hours of available features
            target_features = features_hourly.tail(24)
...
if __name__ == "__main__":
    ...
    # Load and disaggregate a training window
    loader = UKDALELoader()
    loader.load()
